...
*/
    public AST rType() throws SyntaxError {
        ...
        if (isNextTok(Tokens.Int)) {
            t = new IntTypeTree(); //tree has no kids
            scan();
        } else if(isNextTok(Tokens.BOOLean) ){
            t = new BoolTypeTree(); // tree has no kids
            scan(); //get NextToken
        }else{ //type must be float if not int or boolean
            expect(Tokens.Flo);
            t= new FloatTypeTree(); //create Float Type tree with no kids
        }
        return t;
    }
...
*/
    public AST rStatement() throws SyntaxError {
        ...
        if (isNextTok(Tokens.If)) {
            ...
            t.addKid(rBlock());
            return t;
        } // Build tree for 'repeat' Block 'while' E
        if (isNextTok(Tokens.Repeat)) {
            scan(); // get next token
            t = new RepeatTree(); //create Repeat tree
            t.addKid(rBlock()); //Add Block kid to the Repeat tree
            expect(Tokens.While); // next token must be While or there is error
            t.addKid(rExpr()); // Add Expression kid to the Repeat Tree
            return t; // Return Repeat tree
        }
        ...
        if (isNextTok(Tokens.LeftBrace)) {
            return rBlock();
        } // ELSE add name '=' e to the Assign tree
        t = rName();
...
*/
    public AST rFactor() throws SyntaxError {
        ...
        if (isNextTok(Tokens.FLOat)) {  //  -> <float>
            t = new FloatTree(currentToken); //create Float tree
            
            scan(); //get Next token
            return t; // return Float tree
        }
